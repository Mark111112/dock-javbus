...
class OnePondoScraper(BaseScraper):
    """1Pondo网站爬虫类"""
    ...
    def __init__(self):
        ...
        # 基础URL设置
        self.base_url = "https://www.1pondo.tv"
        
        # 详情页URL模板
        self.detail_url_template = "https://www.1pondo.tv/movies/{}"
        
        # API URL模板用于获取详细信息
        self.api_url_template = "https://www.1pondo.tv/dyn/phpauto/movie_details/movie_id/{}.json"
        ...
        self.logger = logging.getLogger('OnePondoScraper')
    
    def clean_movie_id(self, movie_id, five_digit=False):
        """标准化影片ID
        
        Args:
            movie_id: 原始影片ID (例如 123456_001, 1pondo-123456_001)
            five_digit: 不适用于1Pondo，保留参数以兼容接口
        ...
        # 尝试提取数字部分，1Pondo的格式通常是6位数_3位数 (例如 123456_001)
        match = re.search(r'(?:1PONDO[-_]?|ONE[-_]?PONDO[-_]?)?(\d{6})(?:[-_])(\d{3})', movie_id, re.IGNORECASE)
        ...
        # 标准化ID格式
        clean_id = f"1PONDO-{number}"
        
        self.logger.debug(f"清理影片ID: {movie_id} -> {clean_id}")
        return "1PONDO", number, clean_id
    ...
    def _extract_thumbnails(self, json_data, url):
        ...
        try:
            ...
            # 构建缩略图URL，1Pondo通常有5个缩略图
            for i in range(1, 6):
                img_url = f"https://www.1pondo.tv/moviepages/{movie_id}/images/popu/{i}.jpg"
