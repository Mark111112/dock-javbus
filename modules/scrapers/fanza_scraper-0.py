    ...
    def _fetch_video_dmm_content_by_content_id(self, content_id, movie_id=None):
        ...
        try:
            ...
            query = (
                ...
                "    products { id priority priceInclusiveTax licenseType shopName __typename }\n"
                "    priceSummary { lowestPrice lowestSalePrice __typename }\n"
                "    __typename\n"
                "  }\n"
            ...
            price_summary = content.get("priceSummary")
            if isinstance(price_summary, dict):
                result["price_summary"] = {
                    "lowest_price": price_summary.get("lowestPrice"),
                    "lowest_sale_price": price_summary.get("lowestSalePrice")
                }
            ...
            if isinstance(products, list) and products:
                ...
                for product in products:
                    ...
                    simplified = {
                        ...
                        "price_inclusive_tax": product.get("priceInclusiveTax"),
                        "license_type": product.get("licenseType"),
                        "shop_name": product.get("shopName")
                    }
                    available_coupon = product.get("availableCoupon")
                    if isinstance(available_coupon, dict):
                        simplified["available_coupon"] = {
                            "name": available_coupon.get("name"),
                            "discounted_price": available_coupon.get("discountedPrice"),
                            "min_payment": available_coupon.get("minPayment")
                        }
                    simplified_products.append(simplified)
                if simplified_products:
                    result["products"] = simplified_products
