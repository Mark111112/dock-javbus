...
class MusumeScraper(BaseScraper):
    """1Pondo网站爬虫类"""
    ...
    def __init__(self):
        ...
        # 基础URL设置
        self.base_url = "https://www.10musume.com"
        
        # 详情页URL模板
        self.detail_url_template = "https://www.10musume.com/movies/{}/"
        
        # API URL模板用于获取详细信息
        self.api_url_template = "https://www.10musume.com/dyn/phpauto/movie_details/movie_id/{}.json"
        ...
        self.logger = logging.getLogger('MusumeScraper')
    
    def clean_movie_id(self, movie_id, five_digit=False):
        """标准化影片ID
        
        Args:
            movie_id: 原始影片ID (例如 123456_01, 10musume-123456_01)
            five_digit: 不适用于10musume，保留参数以兼容接口
        ...
        # 尝试提取数字部分，1Pondo的格式通常是6位数_2位数 (例如 123456_01)
        match = re.search(r'(?:1PONDO[-_]?|ONE[-_]?PONDO[-_]?)?(\d{6})(?:[-_])(\d{2})', movie_id, re.IGNORECASE)
        ...
        # 标准化ID格式
        clean_id = f"10musume-{number}"
        
        self.logger.debug(f"清理影片ID: {movie_id} -> {clean_id}")
        return "10MUSUME", number, clean_id
    ...
    def extract_info_from_page(self, soup, movie_id, url):
        ...
        try:
            ...
            if 'MovieID' in json_data:
                info['pondo_id'] = json_data['MovieID']
    ...
    def _extract_thumbnails(self, json_data, url):
        ...
        try:
            ...
            # 构建缩略图URL，10Musume通常有5个缩略图
            for i in range(1, 6):
                img_url = f"https://www.10musume.tv/moviepages/{movie_id}/images/popu/{i}.jpg"
