...
class JavbusDatabase:
    ...
    def save_star(self, star_data):
        ...
        支持两种数据来源：
        - javbus: name, avatar, birthday, age, height, bust, waistline, hipline, birthplace, hobby
        - av-league: 额外提供 aliases, debut_date, video_count, solo_count, tags, av_league_comments, instagram_images
        """
        self.ensure_connection()
        try:
            star_id = star_data.get('id')
            if not star_id:
                logger.error("save_star: star_id 为空")
                return False
            ...
            # 检查是否有 av_league_updated 列来判断是否已升级
            has_av_league_columns = 'av_league_updated' in column_names

            logger.debug(f"save_star: star_id={star_id}, has_av_league_columns={has_av_league_columns}, columns={len(column_names)}")

            if not has_av_league_columns:
                # 使用旧版本 SQL（不包含 av-league 字段）
                self.local.cursor.execute('''
                ...
                # 使用新版本 SQL（包含 av-league 字段）
                # 检查是否已有 av-league 数据
                self.local.cursor.execute('''
                SELECT data_source, av_league_updated FROM stars WHERE id = ?
                ''', (star_id,))
                existing = self.local.cursor.fetchone()

                # 确定数据来源
                data_source = star_data.get('data_source', 'javbus')
                av_league_updated = star_data.get('av_league_updated')

                # 如果本次没有 av-league 数据但数据库有，保留原来的
                if data_source != 'av-league' and existing and existing['data_source'] == 'av-league':
                    av_league_updated = existing['av_league_updated']

                self.local.cursor.execute('''
                INSERT OR REPLACE INTO stars
                (id, data, name, avatar, birthday, age, height, bust, waistline, hipline, birthplace, hobby, last_updated,
                 aliases, av_league_avatar, debut_date, video_count, solo_count, tags,
                 av_league_comments, instagram_images, data_source, av_league_updated)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    star_id,
                    data_json,
                    ...
                    now,
                    # av-league 字段
                    json.dumps(star_data.get('aliases', []), ensure_ascii=False) if star_data.get('aliases') else None,
                    star_data.get('av_league_avatar', ''),
                    star_data.get('debut_date', ''),
                    star_data.get('video_count'),
                    star_data.get('solo_count'),
                    json.dumps(star_data.get('tags', []), ensure_ascii=False) if star_data.get('tags') else None,
                    json.dumps(star_data.get('av_league_comments', []), ensure_ascii=False) if star_data.get('av_league_comments') else None,
                    json.dumps(star_data.get('instagram_images', []), ensure_ascii=False) if star_data.get('instagram_images') else None,
                    data_source,
                    av_league_updated
                ))
    ...
    def get_star(self, star_id, max_age=365):
    ...
    def get_star_by_name(self, name: str):
        """根据演员名称查找演员（用于 av-league 匹配）"""
        self.ensure_connection()
        try:
            self.local.cursor.execute('''
            SELECT data FROM stars WHERE name = ?
            ''', (name,))
            ...
            print(f"根据名称查找演员错误: {e}")
            return None

    def update_star_with_av_league_data(self, star_id: str, av_league_data: dict):
        """用 av-league 数据更新现有演员信息

        Args:
            star_id: 演员 ID
            av_league_data: 从 av-league 获取的数据
        ...
        try:
            # 获取现有数据
            self.local.cursor.execute('SELECT data FROM stars WHERE id = ?', (star_id,))
            result = self.local.cursor.fetchone()
            if not result:
                logger.error(f"演员 {star_id} 不存在，无法更新")
                return False

            existing_data = json.loads(result['data'])

            # 合并数据：av-league 数据不覆盖 javbus 的基础字段，只补充新字段
            merged_data = existing_data.copy()

            # av-league 特有字段
            if 'aliases' in av_league_data:
                merged_data['aliases'] = av_league_data['aliases']
            if 'av_league_avatar' in av_league_data:
                merged_data['av_league_avatar'] = av_league_data['av_league_avatar']
            if 'debut_date' in av_league_data:
                merged_data['debut_date'] = av_league_data['debut_date']
            if 'video_count' in av_league_data:
                merged_data['video_count'] = av_league_data['video_count']
            if 'solo_count' in av_league_data:
                merged_data['solo_count'] = av_league_data['solo_count']
            if 'tags' in av_league_data:
                merged_data['tags'] = av_league_data['tags']
            if 'av_league_comments' in av_league_data:
                merged_data['av_league_comments'] = av_league_data['av_league_comments']
            if 'instagram_images' in av_league_data:
                merged_data['instagram_images'] = av_league_data['instagram_images']
            if 'twitter' in av_league_data:
                merged_data['twitter'] = av_league_data['twitter']
            if 'instagram' in av_league_data:
                merged_data['instagram'] = av_league_data['instagram']

            # 补充 javbus 缺失的基础字段（如果有）
            for field in ['birthday', 'age', 'height', 'cup', 'bust', 'waistline', 'hipline', 'birthplace']:
                if not merged_data.get(field) and av_league_data.get(field):
                    merged_data[field] = av_league_data[field]

            # 更新元数据
            merged_data['data_source'] = 'javbus+av-league'
            merged_data['av_league_updated'] = int(time.time())

            # 保存合并后的数据
            return self.save_star(merged_data)

        except Exception as e:
            import traceback
            logger.error(f"更新 av-league 数据错误: {e}")
            logger.error(f"详细错误: {traceback.format_exc()}")
            return False

    def get_star_info_for_display(self, star_id: str) -> dict:
        """获取用于前端显示的演员信息

        Returns:
            dict: 包含所有字段的演员信息，用于显示
        """
        self.ensure_connection()
        try:
            self.local.cursor.execute('''
            SELECT data, aliases, av_league_avatar, debut_date, video_count, solo_count,
                   tags, av_league_comments, instagram_images, data_source, av_league_updated
            FROM stars WHERE id = ?
            ''', (star_id,))
            ...
            if result['aliases']:
                try:
                    data['aliases'] = json.loads(result['aliases'])
                except:
                    data['aliases'] = []
            if result['tags']:
                try:
                    data['tags'] = json.loads(result['tags'])
                except:
                    data['tags'] = []
            if result['av_league_comments']:
                try:
                    data['av_league_comments'] = json.loads(result['av_league_comments'])
                except:
                    data['av_league_comments'] = []
            if result['instagram_images']:
                try:
                    data['instagram_images'] = json.loads(result['instagram_images'])
                except:
                    data['instagram_images'] = []

            data['data_source'] = result['data_source']
            data['av_league_updated'] = result['av_league_updated']

            return data

        except Exception as e:
            print(f"获取演员显示信息错误: {e}")
            return {}

    def get_movie(self, movie_id, max_age=365):
    ...
    def upgrade_schema(self):
        ...
        try:
            ...
            av_league_columns = {
                'aliases': 'TEXT',              # 别名
                'av_league_avatar': 'TEXT',     # av-league 头像
                'debut_date': 'TEXT',           # 出道日期
                'video_count': 'INTEGER',       # 出演数
                'solo_count': 'INTEGER',        # 单体本数
                'tags': 'TEXT',                 # 标签 (JSON)
                'av_league_comments': 'TEXT',   # 评论 (JSON)
                'instagram_images': 'TEXT',     # Instagram 近照 (JSON)
                'data_source': 'TEXT',          # 数据来源
                'av_league_updated': 'INTEGER'  # av-league 数据更新时间
            }

            # 合并所有需要的列
            all_columns = {**required_columns, **av_league_columns}
